...
SENADURIAS_VA_LAYOUT_PATH = os.path.join('Archivos', 'Datos', 'Senadurias', 'senaduriasva_layout.xlsx')
SENADURIAS_VA_VOTOS_PATH = os.path.join('Archivos', 'Datos', 'Senadurias', 'senaduriasva_layout_votos.xlsx')
SENADURIAS_VA_PDF_FORMAT_PATH = os.path.join('Archivos', 'Formatos', 'VA-SEN.pdf')
FONT_PATH = os.path.join('Archivos', 'Fonts', 'LetraBrenda-Regular.otf')
SENADURIAS_VA_PDF_DIR = os.path.join('Actas', 'Senadurias_va', 'PDF')
SENADURIAS_VA_JPG_DIR = os.path.join('Actas', 'Senadurias_va', 'JPG')

# Generar el archivo de votos desde el layout
process_excel_file(SENADURIAS_VA_LAYOUT_PATH, SENADURIAS_VA_VOTOS_PATH)

# Leer el archivo Excel
df = pd.read_excel(SENADURIAS_VA_VOTOS_PATH, dtype={
...
pdf_base_path = SENADURIAS_VA_PDF_FORMAT_PATH

# Crear directorio para guardar los archivos generados
os.makedirs(SENADURIAS_VA_PDF_DIR, exist_ok=True)
os.makedirs(SENADURIAS_VA_JPG_DIR, exist_ok=True)
...
for index, row in df.iterrows():
    try:
        ...
        os.makedirs(os.path.join(SENADURIAS_VA_JPG_DIR, nombre_estado_final), exist_ok=True)
        ...
        #pdf_output_path = os.path.join(SENADURIAS_VA_PDF_DIR, f"{nombre_archivo}.pdf")
        ...
        jpg_path = os.path.join(os.path.join(SENADURIAS_VA_JPG_DIR, nombre_estado_final), f"va_{nombre_estado_final}{nombre_archivo2}.jpg")
...
print(f"PDFs y JPGs generados y guardados en '{SENADURIAS_VA_PDF_DIR}' y '{SENADURIAS_VA_JPG_DIR}'.")
